...
*/        
        ...
        //ImageUtils.pressText2("华人生活", "E:\\Tomcat\\apache-tomcat-6.0.9\\webapps\\HuanRenLife\\image\\files\\12700000120130805201332774950.jpg","E:\\Tomcat\\apache-tomcat-6.0.9\\webapps\\HuanRenLife\\image\\files\\127000001201308052013327749501.jpg", "黑体", 36, Color.white, 30, 0.5f);//测试OK
        
        // 6-给图片添加图片水印：
        ImageUtils.pressImage("E:\\Tomcat\\apache-tomcat-6.0.9\\webapps\\HuanRenLife\\image\\files\\logo.jpg","E:\\Tomcat\\apache-tomcat-6.0.9\\webapps\\HuanRenLife\\image\\files\\12700000120130805201332774950.jpg", "E:\\Tomcat\\apache-tomcat-6.0.9\\webapps\\HuanRenLife\\image\\files\\127000001201308052013327749501.jpg", 0.4f);//测试OK
    }
    /**
     * 缩放图像（按比例缩放）
    ...
    public final static void pressText2(String pressText, String srcImageFile,String destImageFile,
            String fontName, int fontStyle, Color color, int fontSize, float alpha) {
        try {
            File img = new File(srcImageFile);
            Image src = ImageIO.read(img);
            int width = src.getWidth(null);
            ...
            g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP,
                    alpha));
            // 在指定坐标绘制水印文字
            g.drawString(pressText, (width - (getLength(pressText) * fontSize))-100, (height - fontSize)-30);
            g.dispose();
            ImageIO.write((BufferedImage) image, "JPEG", new File(destImageFile));
        } catch (Exception e) {
            e.printStackTrace();
